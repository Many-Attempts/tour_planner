...
import { Tour, TourLog, Difficulty } from '../../../core/models';
...
export class TourDetailComponent implements OnInit {
  ...
  saveLog(): void {
    ...
    if (this.editingLogId) {
      this.tourService.updateLog(this.tour.id, this.editingLogId, request).subscribe(() => {
      ...
      this.tourService.createLog(this.tour.id, request).subscribe(() => {
        this.loadLogs(this.tour!.id);
        this.loadTour(this.tour!.id);
        this.showAddLog = false;
        this.resetLogForm();
  ...
  deleteLog(log: TourLog): void {
    ...
    this.tourService.deleteLog(this.tour.id, log.id).subscribe(() => {
      this.loadLogs(this.tour!.id);
      this.loadTour(this.tour!.id);
    });
  }
